...
public abstract class SemanticExtractionFixture extends SemanticHttpFixture {
    ...
    protected Map<String,Object> edit(long version){Map<String,Object> m=new LinkedHashMap<>();m.put("expectedVersion",version);m.put("operationId",op());m.put("status","OPEN");m.put("subjectIri",SUBJECT_IRI);m.put("subjectTypeIris",Set.of(EQUIPMENT_IRI));m.put("subjectName","机床");m.put("subjectId",subject);m.put("subjectResolution",Map.of("mode","EXISTING","reason","Fixture business identity confirmed against source"));m.put("assertionText",assertion(SUBJECT_IRI));m.put("validityKind","UNKNOWN");m.put("quotes",List.of(Map.of("startCodePoint",0,"endCodePoint",text.codePointCount(0,text.length()),"exactQuote",text)));return m;}
