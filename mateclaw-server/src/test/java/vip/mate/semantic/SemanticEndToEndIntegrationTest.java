...
class SemanticEndToEndIntegrationTest extends SemanticHttpFixture {
    ...
    protected Map<String,Object> definition() {
        Map<String,Object> result = new HashMap<>(super.definition());
        result.put("relations", List.of(Map.of("key","connects","label","连接","description","",
                "sourceTypeKey","Equipment","targetTypeKey","Equipment","multiplicity","MULTI")));
        return result;
    }
    ...
    void pinnedSchemaMultiValueGraphChangeReviewAndSupportWithdrawal() throws Exception {
        ...
        assertEquals("connects",detail.path("definition").path("relations").get(0).path("key").asText());
        call("GET",base,"owner",otherWorkspace,null,404);
        List<String> entities=new ArrayList<>();
        for(String label:List.of("P-101","M-01","M-02"))
            entities.add(call("POST",base+"/entities","member",workspace,Map.of("typeKey","Equipment","displayName",label),200).path("id").asText());
        ...
        for(String target:entities.subList(1,3)) {
            Map<String,Object> relation=new HashMap<>(Map.of("operationId",op(),"subjectId",entities.getFirst(),"predicateKind","RELATION","predicateKey","connects","valueType","ENTITY","targetEntityId",target,"validityKind","INTERVAL","evidenceIds",List.of(evidenceId)));
        ...
        Map<String,Object> content=new HashMap<>(Map.of("operationId",op(),"subjectId",entities.getFirst(),"predicateKind","PROPERTY","predicateKey","voltage","valueType","DECIMAL","value","380","unit","V","validityKind","INTERVAL","evidenceIds",List.of(evidenceId)));
        ...
        assertEquals("380",changeRead.path("content").path("value").asText());
        ...
        content.put("operationId",op()); content.put("value","400");
