CREATE TABLE mate_semantic_source_change_run (
 ...
 status VARCHAR(16) NOT NULL, expected_graph_version BIGINT, request_digest VARCHAR(64) NOT NULL, baseline_json LONGTEXT NOT NULL, created_by VARCHAR(32) NOT NULL,
 created_at TIMESTAMP(6) NOT NULL, updated_at TIMESTAMP(6) NOT NULL,
 UNIQUE KEY uq_sem_source_change_run_operation(graph_id,operation_id)
);
...
CREATE TABLE mate_semantic_source_change (
 ...
 UNIQUE KEY uq_sem_source_change_run_source(run_id,source_kind,source_ref)
);
...
CREATE TABLE mate_semantic_source_change_item (
 ...
 UNIQUE KEY uq_sem_source_change_item(change_id,item_kind,item_id,item_revision,old_snapshot_id)
);
