...
public class BiddingProjectService {
    ...
    public BiddingProjectService(BiddingRepository repository,BiddingAccess access,ObjectMapper json) {
    ...
    public ObjectNode create(BiddingTypes.Scope scope,BiddingTypes.NewProject request) {
        ...
        try { repository.insertOperation(scope.workspaceId(),scope.actorId(),request.operationId(),digest,"{}",now); }
        catch(DuplicateKeyException race) {
            var stored=repository.findOperation(scope.workspaceId(),scope.actorId(),request.operationId());
            if(stored!=null) return replay(stored,digest);
            throw BiddingAccess.error(409,"OPERATION_CONFLICT","Project creation conflicted with another operation");
        }
    ...
    public ObjectNode execute(BiddingTypes.Scope scope,BiddingTypes.Command command) {
        ...
        if(previous!=null) return replay(previous,digest);
        try { repository.insertOperation(scope.workspaceId(),scope.actorId(),command.operationId(),digest,"{}",now()); }
        catch(DuplicateKeyException race) {
            var stored=repository.findOperation(scope.workspaceId(),scope.actorId(),command.operationId());
            if(stored!=null) return replay(stored,digest);
            throw BiddingAccess.error(409,"OPERATION_CONFLICT","Command conflicted with another operation");
        }
