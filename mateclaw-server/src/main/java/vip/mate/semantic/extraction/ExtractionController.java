...
public class ExtractionController {
    ...
    private final JdbcExtractionRepository repo;private final ExtractionCoordinator coordinator;private final SuggestionSubmissionService submissions;private final ExtractionConfiguration.Feature feature;private final ExtractionScheduler scheduler;private final MateClawSourceAdapter sources;private final GraphApplicationService graphs;
    private final OwlAssertionAdapter assertions;private final SuggestionValidator validator;
    public ExtractionController(SemanticAccessService identity,MateClawAccessAdapter access,MateClawContextAdapter context,MateClawModelAdapter models,JdbcExtractionRepository repo,ExtractionCoordinator coordinator,SuggestionSubmissionService submissions,ExtractionConfiguration.Feature feature,ExtractionScheduler scheduler,MateClawSourceAdapter sources,GraphApplicationService graphs,OwlAssertionAdapter assertions){this.identity=identity;this.access=access;this.context=context;this.models=models;this.repo=repo;this.coordinator=coordinator;this.submissions=submissions;this.feature=feature;this.scheduler=scheduler;this.sources=sources;this.graphs=graphs;this.assertions=assertions;this.validator=new SuggestionValidator(assertions);}
    ...
    public R<ExtractionDtos.Capabilities> capabilities(@RequestHeader(value="X-Workspace-Id",required=false)String scope,@PathVariable String graphId){var a=actor(scope,"viewer");access.require(a,graphId,null,Action.READ);GraphRow graph=graphs.requireGraph(a.workspaceId(),graphId,false);var revision=graphs.revision(a.workspaceId(),graphId);var available=models.models();var reasons=unavailableReasons(graph,available);return R.ok(new ExtractionDtos.Capabilities(feature.enabled(),available,revision.getId(),reasons.isEmpty(),reasons,revision.getName(),revision.getVersion()));}
    @PostMapping("/extraction-tasks") @ResponseStatus(HttpStatus.ACCEPTED) @RequireWorkspaceRole("member")
    public R<ExtractionDtos.TaskView> start(@RequestHeader(value="X-Workspace-Id",required=false)String scope,@PathVariable String graphId,@RequestBody ExtractionDtos.StartRequest request){var a=actor(scope,"member");var ref=sources.withGraphLock(a,graphId,()->{GraphRow graph=graphs.requireGraph(a.workspaceId(),graphId,false);var available=models.models();requireStartable(graph,available,request.modelConfigId());return coordinator.start(a,new StartCommand(graphId,request.sourceRef(),request.modelConfigId(),request.operationId(),request.expectedOntologyRevisionId()));});return R.ok(view(visible(a,graphId,ref.taskId())));}
    private List<String> unavailableReasons(GraphRow graph,List<ExtractionDtos.ModelView> available){var reasons=new ArrayList<String>();if(!feature.enabled())reasons.add("EXTRACTION_DISABLED");if(!scheduler.enabled())reasons.add("SCHEDULER_DISABLED");if(!Boolean.TRUE.equals(graph.getEnabled()))reasons.add("BINDING_DISABLED");if(available.isEmpty())reasons.add("MODEL_UNAVAILABLE");return List.copyOf(reasons);}
    private void requireStartable(GraphRow graph,List<ExtractionDtos.ModelView> available,String modelConfigId){var reasons=unavailableReasons(graph,available);if(!reasons.isEmpty()){String code=reasons.getFirst();if("BINDING_DISABLED".equals(code))code="GRAPH_DISABLED";throw new ExtractionException(409,code);}if(modelConfigId==null||available.stream().noneMatch(m->modelConfigId.equals(m.id())))throw new ExtractionException(409,"MODEL_UNAVAILABLE");}
    ...
    public R<ExtractionDtos.TaskView> retry(@RequestHeader(value="X-Workspace-Id",required=false)String scope,@PathVariable String graphId,@PathVariable String taskId,@RequestBody ExtractionDtos.OperationRequest r){var a=actor(scope,"member");Task t=visible(a,graphId,taskId);requireStartable(graphs.requireGraph(a.workspaceId(),graphId,false),models.models(),t.configuration().modelConfigId());if(((Number)repo.metadata(taskId).get("attempts")).intValue()>=3)throw new SemanticApiException(409,"ATTEMPT_LIMIT","Task exhausted three attempts");coordinator.retry(a,graphId,taskId,r.operationId());return R.ok(view(visible(a,graphId,taskId)));}
