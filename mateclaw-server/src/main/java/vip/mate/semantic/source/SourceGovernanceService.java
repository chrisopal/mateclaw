...
public class SourceGovernanceService {
    private final JdbcTemplate jdbc;private final GraphApplicationService graphs;private final SemanticAccessService access;private final OntologyWireMapper wire;
    public SourceGovernanceService(JdbcTemplate jdbc,GraphApplicationService graphs,SemanticAccessService access,OntologyWireMapper wire){this.jdbc=jdbc;this.graphs=graphs;this.access=access;this.wire=wire;}
    @Transactional public GovernanceResult withdraw(String scope,String graphId,GovernanceRequest request){
        ...
        GovernanceResult result=new GovernanceResult(request.sourceKind(),request.sourceRef(),"WITHDRAWN");command(graphId,request,"SOURCE_WITHDRAW",result);return result;
    }
    @Transactional public GovernanceResult exclude(String scope,String graphId,String snapshotId,GovernanceRequest request){
        ...
        GovernanceResult result=new GovernanceResult("SNAPSHOT",snapshotId,"EXCLUDED");command(graphId,request,"SNAPSHOT_EXCLUDE",result);return result;
    }
