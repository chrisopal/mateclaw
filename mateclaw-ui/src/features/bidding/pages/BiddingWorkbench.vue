<template>
  <main class="bidding-workbench" v-loading="loading">
    ...
    <el-alert v-if="conflict && projectDraftChanged" :title="l('服务器版本已刷新；您的项目档案草稿仍保留。','The server version is refreshed; your project settings draft is retained.')" type="warning" :closable="false" />
    ...
    <BiddingTaskDrawer v-model="taskDrawer" :workspace-id="workspace.currentWorkspaceId || ''" :project="project!" :can-write="!!capabilities?.canWrite" v-if="project" @changed="refreshAll" />
...
const projectDraftChanged=computed(()=>!!project.value&&(projectDraft.name!==project.value.name||projectDraft.lotName!==project.value.lotName||projectDraft.ownerId!==String(project.value.ownerId)))
...
async function load(){const ws=workspace.currentWorkspaceId||'',id=projectId.value;const sameProject=project.value?.id===id&&project.value.workspaceId===ws;const preserveDraft=sameProject&&projectDraftChanged.value;controller?.abort();controller=new AbortController();const current=controller,signal=current.signal;if(!sameProject){project.value=undefined;sources.value=[];analysis.value={groups:[]}}error.value='';if(!preserveDraft)conflict.value=false;unavailable.value=false;if(!ws){error.value=l('请选择工作区。','Select a workspace.');return}loading.value=true
try{const caps=await biddingApi.capabilities(ws,signal);if(!assertCurrent(ws,id,signal))return;capabilities.value=caps;if(!caps.enabled){unavailable.value=true;return}const [detail,memberRows,employeeRows]=await Promise.all([biddingApi.get(ws,id,signal),biddingApi.members(ws,signal),biddingApi.employees(ws,signal)]);if(!assertCurrent(ws,id,signal))return;project.value=detail;members.value=memberRows.map(m=>({...m,userId:String(m.userId)}));employees.value=employeeRows.map(e=>({...e,id:String(e.id)}));if(!preserveDraft){projectDraft.name=detail.name;projectDraft.lotName=detail.lotName;projectDraft.ownerId=String(detail.ownerId)}employeeError.value='';await Promise.all([refreshSources(),refreshAnalysis()])}
