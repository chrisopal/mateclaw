      ...
      <el-tab-pane v-for="skill in availableSkills" :key="skill.id" :name="skill.id" :label="skill.label">
        <div v-if="payloadFor(skill.id)" class="result-panel">
          ...
          <div v-if="skill.id === 'bidding-tender-profile'" class="structured-grid">
            ...
            <el-table :data="array(payloadFor(skill.id)?.deadlines)" size="small"><el-table-column prop="name" :label="l('时间事项','Deadline')" min-width="150"/><el-table-column prop="value" :label="l('日期','Date')" width="180"/><el-table-column :label="l('证据','Evidence')" min-width="120"><template #default="{row}"><el-button v-if="row.evidenceRefs?.length" type="primary" link @click="$emit('evidence', row.evidenceRefs[0])">{{ l('查看证据','Evidence') }}</el-button><span v-else>—</span></template></el-table-column></el-table>
          </div>
          <el-table v-else :data="resultRows(skill.id)" size="small" row-key="id" class="result-table">
            ...
            <el-table-column :label="l('证据','Evidence')" min-width="120"><template #default="{row}"><el-button v-if="row.evidenceRefs?.length" type="primary" link @click="$emit('evidence', row.evidenceRefs[0])">{{ l('查看证据','Evidence') }}</el-button><span v-else>—</span></template></el-table-column>
          </el-table>
...
const groupStatus = computed(() => baseline.value ? l('已确认','Confirmed') : activeGroup.value?.status || l('候选','Candidate'))
...
function resultRows(skill:string) { const payload=payloadFor(skill); if(skill==='bidding-elimination-analysis') return array(payload?.items).map(item=>({id:item.id,title:item.text,detail:[item.scope,item.trigger,item.unknowns?.join('; ')].filter(Boolean).join(' · '),score:'',evidenceRefs:item.evidenceRefs})); if(skill==='bidding-requirement-analysis') return array(payload?.requirements).map(item=>({id:item.id,title:item.text,detail:[item.category,item.constraints?.join('; '),item.acceptance?.join('; ')].filter(Boolean).join(' · '),score:'',evidenceRefs:item.evidenceRefs})); return array(payload?.criteria).map(item=>({id:item.id,title:item.title,detail:item.rule,score:item.score==null?l('待核实','Unknown'):`${item.score}${item.unit||''}`,evidenceRefs:item.evidenceRefs})) }
function printable(value:unknown) { return value == null || value === '' ? '—' : String(value) }
