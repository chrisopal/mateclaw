<template>
  <el-drawer v-model="open" :title="l('原文证据','Source evidence')" size="min(560px, 94vw)" @closed="clear">
    ...
    <article v-else-if="evidence"><div class="locator">{{ evidence.locator }}<span v-if="evidence.pdfPage"> · {{ l('第','p.') }}{{ evidence.pdfPage }}</span></div><blockquote>{{ evidence.text }}</blockquote><small>{{ evidence.sourceId }} · V{{ evidence.version }}</small></article>
  </el-drawer>
</template>
...
const open=ref(false),loading=ref(false),error=ref(''),evidence=ref<Evidence>(),controller=ref<AbortController>()
