...
export interface ExtractionCapabilities { enabled: boolean; models: { id: string; name: string }[]; ontologyRevisionId: string; canStart?: boolean; unavailableReasons?: string[]; ontologyName?: string; ontologyVersion?: number }
...
export const extractionApi = {
  ...
  start: (ws: string, g: string, data: { sourceRef: string; modelConfigId: string; operationId: string; expectedOntologyRevisionId?: string }, signal?: AbortSignal) => semanticRequest<ExtractionTask>(ws, { url: base(g), method: 'POST', data }, signal),
