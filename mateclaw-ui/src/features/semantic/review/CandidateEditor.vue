...
async function submit() {
  if (!selected.value || !range.value || !snapshot.value) return
  ...
  try {
    ...
    if (saved) { range.value = null; emit('saved') }
  } catch (e) { if (run.current()) error.value = (e as Error).message }
  finally { if (run.current()) busy.value = false }
}
</script>
...
<el-alert v-if="error" :title="error" type="error" :closable="false" /><el-button native-type="submit" type="primary" :loading="busy" :disabled="!range || !selected || (selected.kind === 'RELATION' ? !targetEntity : !value)">{{ t(target ? 'semantic.w.proposeChange' : 'semantic.w.propose') }}</el-button>
</el-form></template>
