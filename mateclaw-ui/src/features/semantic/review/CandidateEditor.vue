...
<el-form-item :label="t('semantic.w.predicate')"><el-select v-model="predicate"><el-option v-for="p in predicates" :key="p.kind + p.key" :value="`${p.kind}:${p.key}`" :label="`${p.label} · ${t('semantic.values.' + p.multiplicity)}`" /></el-select></el-form-item>
