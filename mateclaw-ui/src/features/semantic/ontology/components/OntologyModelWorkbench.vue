...
const directoryId = `ontology-directory-${useId()}`
...
watch(query, () => { if(query.value)directoryOpen.value=true; void nextTick(() => canvas.value?.focus(view.value.matches[0])) })
  ...
  <div class="ontology-model">
    ...
    <div v-if="!projectionLoading" class="model-layout">
      <aside class="model-directory">
        <div class="model-directory-heading"><label :for="searchId">{{ tr('对象与关系','Objects and relations') }}</label><button class="model-directory-toggle" type="button" :aria-expanded="directoryOpen" :aria-controls="directoryId" @click="directoryOpen=!directoryOpen">{{ directoryOpen?tr('收起目录','Collapse'):tr('展开目录','Show directory') }} · {{nodes.length}}</button></div>
        <el-input :id="searchId" v-model="query" clearable :placeholder="tr('搜索对象或关系','Search objects or relations')" />
        <div :id="directoryId" class="model-directory-content" :class="{'is-open':directoryOpen}">
      ...
      <section class="model-canvas-panel" :aria-label="tr('本体关系图','Ontology graph')">
        <details class="model-canvas-filters"><summary>{{tr('筛选关联','Filter links')}}</summary>
...
.model-directory-heading{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:12px}.model-directory-heading label{margin:0}.model-directory-toggle{display:none}.model-canvas-filters{padding:8px 12px;border-bottom:1px solid var(--mc-border);font-size:12px}.model-canvas-filters>summary{cursor:pointer;color:var(--mc-text-secondary)}.model-canvas-filters .model-canvas-bar{padding:8px 0 0;border:0}.model-directory-toggle:focus-visible{outline:2px solid var(--el-color-primary);outline-offset:2px}@media(max-width:650px){.model-directory{padding:12px}.model-directory-content:not(.is-open){display:none}.model-directory-toggle{display:inline-flex;border:0;padding:4px 0;background:transparent;color:var(--mc-primary);font:inherit;font-size:12px;cursor:pointer}.model-term-list{max-height:180px}.model-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));width:100%;gap:6px}.model-actions .el-button{padding:8px 6px;font-size:12px;min-width:0}.model-canvas-bar{display:grid;grid-template-columns:1fr;gap:8px}.model-canvas-filters .model-edge-filter{justify-content:space-between}}
</style>
