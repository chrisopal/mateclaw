...
import { displayModel, type DisplayProjection } from '../standardProjection'
import { termEdits, type TermInput } from '../ontologyForm'
const props = defineProps<{ axioms: AxiomDescriptor[]; projection?: DisplayProjection | null; projectionLoading?: boolean; projectionError?: string; editable?: boolean; disabled?: boolean; reloadDraft?: () => Promise<boolean>; editPending?: boolean; retryEdit?: () => Promise<boolean>; failureMessage?: string; apply?: (edits: AxiomEdit[]) => Promise<boolean> }>()
...
const model = computed(() => {
 ...
 const labels:Record<string,string>={subClassOf:tr('子类属于','Subclass'),domain:tr('定义域','Domain'),range:tr('值域','Range'),equivalentClasses:tr('等价','Equivalent'),disjointClasses:tr('互斥','Disjoint'),inverseOf:tr('互为逆属性','Inverse properties')}
 return {...model,edges:model.edges.map(e=>({...e,label:labels[e.kind]||e.label}))}
})
...
<template>
  <div class="ontology-model">
    <div class="model-bar">
      <div><strong>{{ tr('本体结构','Ontology structure') }}</strong><span v-if="projection" class="model-count">{{ model.nodes.length }} {{ tr('项定义','definitions') }} · {{ model.edges.length }} {{ tr('条结构关联','structural links') }}</span></div>
    ...
    <el-alert v-if="projectionLoading" :title="tr('正在加载标准投影…','Loading standard projection…')" type="info" :closable="false" />
    <div v-if="projectionError" class="model-projection-error" role="alert"><p>{{tr('标准投影不可用，下面保留原公理。可重新加载或打开高级视图。','Standard projection unavailable. Original axioms remain below; retry or open the advanced view.')}} {{projectionError}}</p><el-button @click="emit('reload-projection')">{{tr('重新加载投影','Reload projection')}}</el-button></div>
    <div v-if="!projectionLoading" class="model-layout">
      ...
      <aside class="model-inspector" :class="{'is-open': inspectorOpen}">
        ...
        <details v-for="rule in rules" :key="rule.id" class="model-rule"><summary>{{ ruleLabel(rule.axiomType) }}</summary><p v-if="rule.imported">{{tr('固定导入（只读）','Pinned import (read only)')}} · {{rule.artifactId}}</p><p v-if="rule.reason" class="model-note">{{rule.status}} · {{rule.reason}}</p><pre>{{ rule.rendering }}</pre><el-button v-if="!rule.imported" link type="primary" @click="emit('inspect-axiom',rule.axiomId); inspectorOpen=false">{{ tr('查看公理与来源','Inspect axiom and sources') }}</el-button></details>
