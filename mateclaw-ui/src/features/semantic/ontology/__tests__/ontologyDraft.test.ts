...
function setup() {
  ...
  const api = { getDraft: vi.fn().mockResolvedValue(source()), saveDraft: vi.fn(), latestValidation: vi.fn().mockResolvedValue(null), validate: vi.fn(), publish: vi.fn(), operation: vi.fn(), discard: vi.fn(), editDraft: vi.fn(), retryEdit: vi.fn(), modelEdit: vi.fn() } as unknown as OntologyApi
  const scope = effectScope()
  const draft = scope.run(() => useOntologyDraft(() => source().ontologyId, () => ws.value, api))!
  return { ws, api, scope, draft }
}
const passed = { reportId: 'report', inputDigest: 'digest', stale: false, checks: requiredChecks.map(kind => ({kind,status:'PASS',violations:[]})) }
...
describe('ontology document draft', () => {
  ...
  it('expires validation immediately on document edits and keeps string IDs', async () => {
    const { draft, api, scope } = setup(); await draft.load(); vi.mocked(api.validate).mockResolvedValue({ ...passed, draftVersion: 17, valid: true, violations: [] }); await draft.validate()
  ...
  it('keeps publishing enabled when validation only reports warnings', async () => {
    const { draft, api, scope } = setup(); await draft.load(); vi.mocked(api.validate).mockResolvedValue({ ...passed, draftVersion: 17, valid: true, violations: [{ severity: 'WARNING', code: 'OWL_ANNOTATION', path: 'document', message: 'annotation' }] }); await draft.validate()
...
describe('publication recovery', () => {
  it('clears terminal publication conflict so refreshed draft uses a fresh operation', async () => {
    const { draft, api, scope } = setup(); await draft.load(); vi.mocked(api.validate).mockResolvedValue({ ...passed, draftVersion: 17, valid: true, violations: [] }); await draft.validate(); vi.mocked(api.publish).mockRejectedValueOnce({ status: 409, code: 'DRAFT_CONFLICT' }); await draft.publish('first')
    const first = vi.mocked(api.publish).mock.calls[0][2]; expect(draft.publicationPending.value).toBe(false); vi.mocked(api.getDraft).mockResolvedValue({ ...source(), draftVersion: 24 }); await draft.load(); vi.mocked(api.validate).mockResolvedValue({ ...passed, draftVersion: 24, valid: true, violations: [] }); await draft.validate(); vi.mocked(api.publish).mockRejectedValueOnce({ status: 422, code: 'VALIDATION_FAILED' }); await draft.publish('second')
...
it('does not accept a validation result returned after input changed', async () => {
  const { draft, api, scope } = setup(); await draft.load(); let resolve!: (report: { draftVersion: number; valid: boolean; violations: [] }) => void; vi.mocked(api.validate).mockImplementationOnce(() => new Promise(r => { resolve = r })); const pending = draft.validate(); draft.name.value = 'More recent local input'; resolve({ ...passed, draftVersion: 17, valid: true, violations: [] }); expect(await pending).toBe(false); expect(draft.validationReport.value).toBeNull(); expect(draft.canPublish.value).toBe(false); scope.stop()
})
...
describe('simple edit write boundary', () => {
  ...
  it('blocks edits while publication recovery is pending', async () => {
    ...
    vi.mocked(api.validate).mockResolvedValue({ ...passed, draftVersion: 17, valid: true, violations: [] }); await draft.validate()
...
it('reads the persisted report after reload and never authorizes a stale report',async()=>{
 const {draft,api,scope}=setup()
 vi.mocked(api.latestValidation).mockResolvedValue({...passed,draftVersion:17,valid:true,violations:[]})
 await draft.load();expect(draft.canPublish.value).toBe(true)
 vi.mocked(api.latestValidation).mockResolvedValue({...passed,draftVersion:17,valid:true,violations:[],stale:true})
 await draft.load();expect(draft.canPublish.value).toBe(false);scope.stop()
})
it('revokes an earlier success before a new failed check',async()=>{
 const {draft,api,scope}=setup();await draft.load()
 vi.mocked(api.validate).mockResolvedValue({...passed,draftVersion:17,valid:true,violations:[]})
 await draft.validate();expect(draft.canPublish.value).toBe(true)
 vi.mocked(api.validate).mockRejectedValue({status:504,code:'TIMEOUT'})
 await draft.validate();expect(draft.canPublish.value).toBe(false);expect(draft.validationReport.value).toBeNull();scope.stop()
})
