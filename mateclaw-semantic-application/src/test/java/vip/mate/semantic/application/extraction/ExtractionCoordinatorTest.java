...
class ExtractionCoordinatorTest {
    ...
    final OntologyRevision ontology=new OntologyRevision(new OntologyRevisionId("5"),new OntologyId("6"),1,
            new OntologyDefinition(List.of(new EntityTypeDefinition("Equipment","Equipment","")),
            List.of(new PropertyDefinition("voltage","Voltage","","Equipment",ValueType.DECIMAL,Multiplicity.SINGLE,Optional.of("V"))),List.of()));
    ...
    final class Harness {
        ...
        ExtractionCoordinator coordinator(String text,ExtractionModelPort model,Clock clock,BooleanSupplier enabled){return new ExtractionCoordinator((a,g,s)->new SourceSnapshotInput("9",s,"digest",text,Map.of()),(a,g,s,action)->{},context,repository,model,clock,enabled);}
    }
    ...
    @Test void quoteCannotEscapeItsModelChunk(){Harness h=new Harness();String text="甲".repeat(6001);AtomicInteger chunks=new AtomicInteger();var c=h.coordinator(text,r->{chunks.incrementAndGet();return new ModelResult(List.of(new RawSuggestion(new ObjectMention("machine","Equipment","M"),PredicateRef.property("voltage"),new StatementValue.DecimalValue("220","V"),null,Validity.unknown(),List.of(new Quote(5999,6001,"甲甲")))),new Usage(1,1),"");},Clock.fixed(now,ZoneOffset.UTC),()->true);c.start(actor,new StartCommand("4","10","7","op"));c.runNext("worker");assertEquals(2,chunks.get());assertNotNull(h.results);assertTrue(h.results.getFirst().diagnostics().stream().anyMatch(v->v.code().equals("QUOTE_MISMATCH")));}
