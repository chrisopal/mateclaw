...
class ExtractionCoordinatorTest {
    ...
    @Test void staleExpectedOntologyRevisionIsRejectedBeforeTaskInsert(){Harness h=new Harness();var c=h.coordinator("source",r->new ModelResult(List.of(),new Usage(0,0),""),Clock.fixed(now,ZoneOffset.UTC),()->true);assertThrows(ExtractionException.class,()->c.start(actor,new StartCommand("4","10","7","op","stale-revision")));assertNull(h.task);}
}
